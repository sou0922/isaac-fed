...
# =====================================================
NUM_ROBOTS = 4
RUN_TIME = 500.0              # ウォームアップ2秒 + 実行（人が周回するため長め）
...
async def main():
    ...
    side_camera = Camera(
        ...
        resolution=(1920, 1080),
        frequency=30.0,
        translation=np.array([-90.0, 0.0, 20.0]),  # より後ろ、上、高い位置に
    )
    ...
    if side_cam_prim and side_cam_prim.IsValid():
        ...
        if rotate_ops:
            rotate_ops[0].Set(Gf.Vec3d(75.0, 0.0, -90.0))
        else:
            rotate_op = xformable.AddRotateXYZOp()
            rotate_op.Set(Gf.Vec3d(75.0, 0.0, -90.0))
        print(f"[CAMERA] Side camera rotation set to (75, 0, -90)")
